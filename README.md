...
3- When the account is opened, isActive is set to true. There is also a minimum deposit amount of 500 to open an account 
(which means that the balance will start at 500)

4- Customer can only request a loan if there is no loan yet. If that condition is met, 
...
5- When the customer pays the loan, the opposite happens: the money is taken from the balance, 
...
6- Customer can only close an account if there is no loan, AND if the balance is zero. 
